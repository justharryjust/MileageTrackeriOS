...
final class NotificationManager: NSObject, UNUserNotificationCenterDelegate {
    ...
    func scheduleWeeklySummary(weekKm: Double, businessCount: Int, valueDollars: Double, currencyCode: String = "USD") {
        ...
        if businessCount == 0 {
            ...
            let valStr: String
            if valueDollars > 0 {
                let fmt = MileageCalculator.currencyFormatter(for: currencyCode)
                valStr = " · \(fmt.string(from: NSNumber(value: valueDollars)) ?? "")"
            } else {
                valStr = ""
            }
    ...
    func refreshWeeklySummary(tripRepo: TripRepository, mileageCalculator: MileageCalculator, profileRepo: UserProfileRepository) {
        ...
        scheduleWeeklySummary(weekKm: weeklyKm, businessCount: weeklyCount, valueDollars: weeklyDollars, currencyCode: profileRepo.profile.jurisdiction.currencyCode)
    }
