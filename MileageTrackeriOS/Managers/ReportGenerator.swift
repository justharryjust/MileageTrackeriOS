...
final class ReportGenerator {
    ...
    func exportCSV(
        ...
        let currencyCode = profile.jurisdiction.currencyCode
        let currencyFmt = MileageCalculator.currencyFormatter(for: currencyCode)
        ...
        csv += "Total Value,\(currencyFmt.string(from: NSNumber(value: totals.totalValue)) ?? "")\n"
        ...
        TripLogger.shared.log("CSV report generated: \(filename) — \(sorted.count) trips, \(currencyFmt.string(from: NSNumber(value: totals.totalValue)) ?? "")", category: .trip)
        return url
    }
    ...
    func exportPDF(
        ...
        let unit = profile.distanceUnit.shortName
        let currencyFmt = MileageCalculator.currencyFormatter(for: profile.jurisdiction.currencyCode)
        let totals = computeRunningTotals(trips: sorted, calculator: calculator, profile: profile, baseCumulativeKm: baseCumulativeKm)
        ...
        try? renderer.writePDF(to: url) { ctx in
            ...
            func drawTripRow(_ trip: Trip, index: Int, at y: CGFloat) -> CGFloat {
                ...
                let valueStr = currencyFmt.string(from: NSNumber(value: value)) ?? ""
            ...
            func drawSummary(at y: CGFloat) -> CGFloat {
                ...
                let summaryLines = [
                    ...
                    "Total Value:         \(currencyFmt.string(from: NSNumber(value: totals.totalValue)) ?? "")"
                ]
        ...
        TripLogger.shared.log("PDF report generated: \(filename) — \(sorted.count) trips, \(currencyFmt.string(from: NSNumber(value: totals.totalValue)) ?? "")", category: .trip)
        return url
    }
